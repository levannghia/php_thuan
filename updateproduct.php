...
if(!empty($_POST['productName']) && !empty($_POST['productDescription']) && !empty($_POST['productPrice']) && !empty($_FILES['productPhoto'])) {
    $foderPath = 'public/images/' . time() . $_FILES['productPhoto']['name'];
    if(is_uploaded_file($_FILES['productPhoto']['tmp_name']) && move_uploaded_file($_FILES['productPhoto']['tmp_name'], $foderPath)){
        ...
        $productPhoto = time().$_FILES['productPhoto']['name'];
    ...
    <div class="container">
        ...
        <form action="updateproduct.php?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="file" name="productPhoto" id="productPhoto" class="form-control" placeholder="Product photo" aria-describedby="helpId">
            </div>
